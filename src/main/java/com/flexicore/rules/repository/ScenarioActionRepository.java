...
public class ScenarioActionRepository extends AbstractRepositoryPlugin {
	...
	public List<ScenarioAction> listAllScenarioActions(
		...
		QueryInformationHolder<ScenarioAction> queryInformationHolder = new QueryInformationHolder<>(
				filter, ScenarioAction.class, securityContext);
		return getAllFiltered(queryInformationHolder, preds, cb, q, r);
	}

	private void addScenarioActionPredicate(List<Predicate> preds,
			Root<ScenarioAction> r, CriteriaQuery<?> q, CriteriaBuilder cb,
			ScenarioActionFilter filter, SecurityContext securityContext) {
		ScenarioFilter scenarioFilter = filter.getScenarioFilter();
		if (scenarioFilter != null) {
			Subquery<String> sub = getConnectedScenarioActionsSubQuery(q, cb,
					securityContext, scenarioFilter);
			preds.add(filter.isConnected()
					? r.get(ScenarioAction_.id).in(sub)
					: cb.not(r.get(ScenarioAction_.id).in(sub)));
		}

	}

	private Subquery<String> getConnectedScenarioActionsSubQuery(
		...
		Join<Scenario, ScenarioToAction> join = sr
				.join(Scenario_.scenarioToActions);
		Join<ScenarioToAction, ScenarioAction> join2 = join
	...
	public long countAllScenarioActions(ScenarioActionFilter filter,
