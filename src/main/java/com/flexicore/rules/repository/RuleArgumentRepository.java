...
public class RuleArgumentRepository extends AbstractRepositoryPlugin {
    ...
    private void addRuleArgumentsPredicate(List<Predicate> preds, CriteriaQuery<?> q, Root<FlexiCoreRuleArgument> r, CriteriaBuilder cb, RuleArgumentFilter filter, SecurityContext securityContext) {
        RulesFilter rulesFilter = filter.getRulesFilter();
        if (rulesFilter != null) {
            Subquery<String> sub = getConnectedRulesSubQuery(q, cb, securityContext, rulesFilter);
            preds.add(filter.isConnected()?r.get(FlexiCoreRuleArgument_.id).in(sub):cb.not(r.get(FlexiCoreRuleArgument_.id).in(sub)));
        }

    }

    private Subquery<String> getConnectedRulesSubQuery(CriteriaQuery<?> q, CriteriaBuilder cb, SecurityContext securityContext, RulesFilter ruleFilter) {
        ...
        QueryInformationHolder<FlexiCoreRule> queryInformationHolder = new QueryInformationHolder<>(ruleFilter, FlexiCoreRule.class, securityContext);
        prepareQuery(queryInformationHolder, subPreds, cb, sub, sr);
        Join<FlexiCoreRule, RuleToArgument> join = sr.join(FlexiCoreRule_.ruleToExecutionParameters);
        Join<RuleToArgument, FlexiCoreRuleArgument> join2 = join.join(RuleToArgument_.flexiCoreRuleArgument);
