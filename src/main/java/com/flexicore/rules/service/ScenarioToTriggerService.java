...
public class ScenarioToTriggerService implements ServicePlugin {
	...
	private boolean updateScenarioToTriggerNoMerge(ScenarioToTrigger scenarioToTrigger, ScenarioToTriggerCreate creationContainer) {
		boolean update = baseclassNewService.updateBaseclassNoMerge(creationContainer,scenarioToTrigger);
		if (creationContainer.getFiring() != null && !creationContainer.getFiring().equals(scenarioToTrigger.isFiring())) {
			scenarioToTrigger.setFiring(creationContainer.getFiring());
			update = true;
		}

		if (creationContainer.getOrdinal() != null && !creationContainer.getOrdinal().equals(scenarioToTrigger.getOrdinal())) {
			scenarioToTrigger.setOrdinal(creationContainer.getOrdinal());
			update = true;
		}
