...
public class ScenarioManager implements ServicePlugin {
	...
	public void handleTrigger(ScenarioTriggerEvent scenarioTriggerEvent) {
		...
		if (scenarioTriggerEvent.getScenarioTrigger() == null) {
			logger.warning("Scenario Trigger Event " + scenarioTriggerEvent
					+ " had null trigger , ignoring");
			return;
		}
		SecurityContext securityContext = scenarioTriggerEvent
				.getSecurityContext();
		ScenarioTrigger scenarioTrigger = scenarioTriggerEvent
				.getScenarioTrigger();
		Map<String, Scenario> activeScenarios = scenarioToTriggerService
				.listAllScenarioToTrigger(
						new ScenarioToTriggerFilter()
								.setNonDeletedScenarios(true)
								.setEnabled(true)
								.setScenarioTriggers(
										Collections
												.singletonList(scenarioTrigger)),
						securityContext)
				.stream()
				.filter(f -> f.getTriggerManager() == null
						|| rulesService.evaluateTriggerManager(f,
								scenarioTriggerEvent))
				.map(ScenarioToTrigger::getScenario)
				.filter(distinctByKey(Baseclass::getId))
				.filter(f -> f.getFlexiCoreRule() != null
						&& rulesService.evaluateRule(
								new EvaluateRuleRequest()
										.setScenario(f)
										.setScenarioTriggerEvent(
												scenarioTriggerEvent)
										.setRule(f.getFlexiCoreRule()),
								securityContext).isResult())
				.collect(Collectors.toMap(f -> f.getId(), f -> f, (a, b) -> a));
		if (!activeScenarios.isEmpty()) {
			logger.info("Trigger had caused activated scenarios: "
					+ activeScenarios.values().parallelStream()
							.map(Baseclass::getId)
							.collect(Collectors.joining(",")));
			Map<String, List<ScenarioAction>> scenarioActions = scenarioToActionService
					.listAllScenarioToAction(
							new ScenarioToActionFilter().setEnabled(true)
									.setScenarios(
											new ArrayList<>(activeScenarios
													.values())),
							securityContext)
					.parallelStream()
					.filter(f -> f.getScenarioAction().getDynamicExecution() != null)
					.collect(
							Collectors.groupingBy(f -> f.getScenario().getId(),
									Collectors.mapping(
											f -> f.getScenarioAction(),
											Collectors.toList())));
			Map<String, ScenarioAction> executedActions = new HashMap<>();
			for (Map.Entry<String, List<ScenarioAction>> entry : scenarioActions
					.entrySet()) {
				Map<String, ScenarioAction> actionsToExecute = entry
						.getValue()
						.parallelStream()
						.collect(
								Collectors.toMap(f -> f.getId(), f -> f,
										(a, b) -> a));
				Map<String, ExecuteInvokerRequest> executeInvokerRequests = actionsToExecute
						.values()
						.parallelStream()
						.collect(
								Collectors.toMap(
										f -> f.getId(),
										f -> dynamicInvokersService.getExecuteInvokerRequest(
				...
				if (scenario.getActionManagerScript() != null) {
					logger.info("scenario " + scenario.getName() + "("
							+ scenario.getId() + ") has action manager");
					filtered = rulesService.evaluateActionManager(
							scenarioTriggerEvent, scenario, filtered,
							securityContext);
				}
				if (filtered == null) {
				...
				for (Map.Entry<String, ExecuteInvokerRequest> executeInvokerRequestEntry : filtered
						.entrySet()) {
					ExecuteInvokerRequest executeInvokerRequest = executeInvokerRequestEntry
							.getValue();
					ExecuteInvokersResponse response = dynamicInvokersService
							.executeInvoker(executeInvokerRequest,
									securityContext);
					ScenarioAction scenarioAction = actionsToExecute
							.get(executeInvokerRequestEntry.getKey());
					logger.info("invocation of scenario action "
							+ scenarioAction.getId() + "resulted in "
							+ response);
					executedActions.put(scenarioAction.getId(), scenarioAction);
				}
			}
			ScenarioEvent scenarioEvent = scenarioTriggerEvent
					.getScenarioEvent();
			if (scenarioEvent != null) {
				scenarioEvent.setExecutedActions(executedActions.keySet());
				scenarioEvent
						.addToHumanReadableString("Executed Actions were: "
								+ executedActions.values().stream()
										.map(f -> f.getName())
										.collect(Collectors.joining(",")));
				scenarioEvent.setScenarioHints(activeScenarios.values()
						.parallelStream().map(Scenario::getScenarioHint)
