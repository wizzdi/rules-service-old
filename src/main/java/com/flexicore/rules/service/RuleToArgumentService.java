...
public class RuleToArgumentService implements ServicePlugin {
	...
	public void validate(
			RuleToArgumentHolderFilter rulesToExecutionParametersFilter,
			SecurityContext securityContext) {
		Set<String> ruleIds = rulesToExecutionParametersFilter.getRulesIds();
		Map<String, FlexiCoreRule> ruleMap = ruleIds.isEmpty()
				? new HashMap<>()
				: repository
						.listByIds(FlexiCoreRule.class, ruleIds,
								securityContext).parallelStream().parallel()
						.collect(Collectors.toMap(f -> f.getId(), f -> f));
		ruleIds.removeAll(ruleMap.keySet());
		if (!ruleIds.isEmpty()) {
			throw new BadRequestException("No Rules with ids " + ruleIds);
		}
		rulesToExecutionParametersFilter.setRules(new ArrayList<>(ruleMap
				.values()));

	}

	public void validate(RuleToArgumentCreate creationContainer,
			SecurityContext securityContext) {
		String flexicoreRuleArgumentId = creationContainer
				.getFlexicoreRuleArgumentId();
		FlexiCoreRuleArgument flexiCoreRuleArgument = flexicoreRuleArgumentId != null
				? getByIdOrNull(flexicoreRuleArgumentId,
						FlexiCoreRuleArgument.class, null, securityContext)
				: null;
		if (flexiCoreRuleArgument == null && flexicoreRuleArgumentId != null) {
			throw new BadRequestException("No FlexiCoreRuleArgument with id "
					+ flexicoreRuleArgumentId);
		}
	...
	public RuleToArgument createRulesToExecutionParameter(
			RuleToArgumentCreate creationContainer,
			SecurityContext securityContext) {
		RuleToArgument flexiCoreRulesToExecutionParameter = createRulesToExecutionParameterNoMerge(
	...
	public RuleToArgument updateRulesToExecutionParameter(
			RuleToArgumentUpdate creationContainer,
			SecurityContext securityContext) {
		RuleToArgument flexiCoreRulesToExecutionParameter = creationContainer
				.getRuleToArgument();
		if (updateRulesToExecutionParameterNoMerge(
	...
	private boolean updateRulesToExecutionParameterNoMerge(
			RuleToArgument flexiCoreRulesToExecutionParameter,
			RuleToArgumentCreate creationContainer) {
		boolean update = false;
		if (creationContainer.getOrdinal() != null
				&& creationContainer.getOrdinal() != flexiCoreRulesToExecutionParameter
						.getOrdinal()) {
			flexiCoreRulesToExecutionParameter.setOrdinal(creationContainer
					.getOrdinal());
			update = true;
		}
		if (creationContainer.getName() != null
				&& !creationContainer.getName().equals(
						flexiCoreRulesToExecutionParameter.getName())) {
			flexiCoreRulesToExecutionParameter.setName(creationContainer
					.getName());
			update = true;
		}
		...
		if (creationContainer.getDescription() != null
				&& !creationContainer.getDescription().equals(
						flexiCoreRulesToExecutionParameter.getDescription())) {
			flexiCoreRulesToExecutionParameter.setDescription(creationContainer
					.getDescription());
			update = true;
		}

		if (creationContainer.getFlexiCoreRule() != null
				&& (flexiCoreRulesToExecutionParameter.getFlexiCoreRule() == null || !creationContainer
						.getFlexiCoreRule().equals(
								flexiCoreRulesToExecutionParameter
										.getFlexiCoreRule()))) {
			flexiCoreRulesToExecutionParameter
					.setFlexiCoreRule(creationContainer.getFlexiCoreRule());
			update = true;
		}

		if (creationContainer.getFlexiCoreRuleArgument() != null
				&& (flexiCoreRulesToExecutionParameter
						.getFlexiCoreRuleArgument() == null || !creationContainer
						.getFlexiCoreRuleArgument().equals(
								flexiCoreRulesToExecutionParameter
										.getFlexiCoreRuleArgument()))) {
			flexiCoreRulesToExecutionParameter
					.setFlexiCoreRuleArgument(creationContainer
							.getFlexiCoreRuleArgument());
			update = true;
		}
		return update;
	...
	public PaginationResponse<RuleToArgument> getAllRulesToExecutionParameters(
		...
		long count = repository
				.countAllRuleToArguments(filter, securityContext);
		return new PaginationResponse<>(list, filter, count);
	}

	public List<RuleToArgument> listRuleToArgument(
			RuleToArgumentHolderFilter filter, SecurityContext securityContext) {
